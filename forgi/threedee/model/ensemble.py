...
def overall_analysis(y, measure_name="???"):
    ...
    for start in range(0, len(y)-window_length//2, window_length):
        try:
            ie = pymbar.timeseries.statisticalInefficiency(y[start:start+window_length])
        except pymbar.utils.ParameterError:
            ie = window_length
        ineffs.append(ie) 
    ...
    for split in range(1, 7):
        ...
        indices = sorted(new_indices)
    plt_x = [ 0,len(y)//3, len(y)//3, 2*len(y)//3, 2*len(y)//3, len(y) ]
    plt_y = [ pymbar.timeseries.statisticalInefficiency(y[0 : len(y)//3]), pymbar.timeseries.statisticalInefficiency(y[0 : len(y)//3]),
              pymbar.timeseries.statisticalInefficiency(y[len(y)//3 : 2*len(y)//3]), pymbar.timeseries.statisticalInefficiency(y[len(y)//3 : 2*len(y)//3]),
    ...
    for start in range(0, len(y), 700):
        for end in range(start, len(y),  700):
            neff = [(end-start)/pymbar.timeseries.statisticalInefficiency(y[start:end]) ]
    ...
    if True:
        for step in [1000,2000]:
            ...
            for start in range(0,len(y)-step,step):
                neffs+= [(start)/pymbar.timeseries.statisticalInefficiency(y[:start]) ] 
                neff_back += [(len(y)-start)/pymbar.timeseries.statisticalInefficiency(y[start:]) ] 
                plt_x+=[ start]
                if (start%500)==0:
                    print(start, "*", end="")
            ...
            ax[2].plot(plt_x, np.array(neffs)+np.array(neff_back), label=step)
        ax[1].legend()
    plt.show()
    sys.exit()
    ...
    for start in range(0, len(y)-ineff,ineff):
        ...
        plt_y += [ pymbar.timeseries.statisticalInefficiency(y[start: start+ineff]) ] *2
    ax.plot(plt_x, plt_y)
    plt.show()
    sys.exit()
    ...
    for y in (x[::-1], np.concatenate((x[30000:60000],x[60000:],x[:30000])), x):
        ...
        print(indices)
        for i in indices:
            ax[0].plot(np.arange(old_i, i), y[old_i:i], label="{}-{}".format(old_i,i))
            old_i=i
        if indices:
    ...
    for y in (x[::-1], np.concatenate((x[30000:60000],x[60000:],x[:30000])), x):
        ...
        for start in range(2*windowsize,len(y)-windowsize,windowsize//2):
            ...
            neffs.append((start+windowsize)/pymbar.timeseries.statisticalInefficiency(y[:start+windowsize]))
            ...
            if (start%5000)==(2*windowsize%5000):
                print("*", end="")#print(start, z_score, mean_sofar, mean_window, std_sofar, math.sqrt(windowsize))
            #if abs(z_score)>0.05:
            #   break
        ...
        ax[1,0].plot(np.arange(len(neffs))*windowsize+2*windowsize, neffs)
        ...
        ax[1,1].plot(np.arange(1200+windowsize), unbiased2/ynorm2, label="After Step")
        ax[1,1].plot([0,1200], [(unbiased2/ynorm2)[:1200].mean(), (unbiased2/ynorm2)[:1200].mean()], label="Mean for first part, after step")
...
def plot(y, measure_name = "", ax=None):
    ...
    if integr_corr_time>0:    
        ax[0,0].plot(np.arange(len(y[:integr_corr_time])), y[:integr_corr_time], label="burn-in", color="red")
        ax[0,0].plot([integr_corr_time],y[integr_corr_time], "ro", markersize=5)
    if integr_corr_time<len(y):
        ax[0,0].plot(np.arange(len(y[:integr_corr_time]), len(y)), y[integr_corr_time:], label="production", color="blue")
    ...
    N = A_n.size
    while (t < N - 1):
        C = np.sum(dA_n[0:(N - t)] * dA_n[t:N] + dA_n[0:(N - t)] * dA_n[t:N]) / (2.0 * float(N - t) * sigma2_AA) 
        corr_chodera[t] = C
        if (C <= 0.0) and (t > 3):
            break
        t += increment
    ...
    ax[1,0].plot(np.arange(len(corr)), corr, label="whole dataset", color="violet")
    ax[1,0].plot(np.arange(len(corr_chodera)), corr_chodera, color="lightblue", label="pymbar (production)")
    ...
    # BLOCK AVERAGE
    if len(burn_in)>0:
        ax[2,0].plot(np.arange(len(burn_in)), burn_in, "o-", label="burn-in", color="red")
    if len(production)>0:
        ax[2,0].plot(np.arange(len(production))+len(burn_in), production, "o-", label="production", color="blue")
    ax[2,0].set_xlabel("effective step")
    ax[2,0].set_ylabel("Block average of "+measure_name)
    ax[2,0].set_title("Block average for {} independent samples".format(len(production)+len(burn_in)))
    ax[2,0].legend()
    ...
    ax[1,1].plot(np.arange(len(corr_chodera)), corr_chodera, label="pymbar (production)")
    ...
    if len(production)>0:
        ax[2,1].plot(np.arange(len(production))+len(burn_in), production, "o-", label="production")
    ax[2,1].set_xlabel("effective step")
    ax[2,1].set_ylabel("Block average of Delta "+measure_name)
    ax[2,1].set_title("Block average for {} independent samples".format(len(production)+len(burn_in)))
    ax[2,1].legend()
...
def split_and_bin_timeseries(y, skip = None):
    ...
    production = blocked_average_reverse(y[t*skip:], math.ceil(g*skip))[::-1]
    ...
    def get_descriptor(self, descriptor, domain=None):
        ...
        if domain is None:
            if descriptor not in self._descriptors:
                self._descriptors[descriptor] = self._get_descriptor(descriptor, domain)
...
def damped_randomwalk(start, length, mean=0, std=1, damp = 0.9, transition = 1000):
    ...
    for i in range(1,length):
        if i<transition:
            x[i]=x[i-1]*damp+(np.random.randn()*std+mean*(i/transition))
        else:
            ...
            for line in f:
                ...
                if rog_field is None: #The header line
                    for i, field in enumerate(fields):
                        ...
                        print("Data seems not to contain any ROG field. Exiting",file=sys.stderr)
                        sys.exit()
                else:
                    ...
                    try:
                        assert f[1] == "A" #Not an ROG Field? Expected FLOAT-SPACE-"A" format
                    except:
                        print(f)
                        raise
