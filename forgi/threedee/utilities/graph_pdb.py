...
def get_bulge_centroid(chain, define):
    ...
    res_nums = []
    while i < len(define):
        res_nums += list(range(int(define[i]), int(define[i + 1]) + 1))
        i += 2
...
def get_mids(cg, chain, define, method=cc.Configuration.mids_method, seq_ids=True):
    ...
    if method == 'template':
        ...
        print("Unknown mids method:", method, file=sys.stderr)
        sys.exit(1)
...
def bounding_boxes(bg, chain, s, i):
    ...
    for k in range(2):
        ...
        for atom in list(atoms[k].values()):
            for j in range(3):
...
class VirtualAtomsLookup(object):
    ...
    def __getitem__(self, position):
        ...
        for key, value in list(self.cg.defines.items()):
            if len(value)<2:
    ...
    def keys(self):
        k=set()
        for value in list(self.cg.defines.values()):
            if len(value)>1:
