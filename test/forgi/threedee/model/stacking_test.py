...
class TestAllFilesTyagi(unittest.TestCase):
    def bulge_from_seqids(self, cg, bp1, bp2, asserts=True):
        try:
            ...
            if asserts: self.assertEqual(cg.pairing_partner(cg.seq_id_to_pos(bp1[0])),cg.seq_id_to_pos(bp1[1]))
            if asserts: self.assertEqual(len(stem1), 1)
            stem1, =stem1
            stem2 = cg.nucleotides_to_elements([cg.seq_id_to_pos(bp2[0]), cg.seq_id_to_pos(bp2[1])])
            if asserts: self.assertEqual(cg.pairing_partner(cg.seq_id_to_pos(bp2[0])),cg.seq_id_to_pos(bp2[1]))
            if asserts: self.assertEqual(len(stem2), 1)
            stem2, =stem2
            if asserts: self.assertNotEqual(stem1, stem2)
            bulges = (cg.edges[stem1] & cg.edges[stem2])
            if asserts: self.assertGreaterEqual(len(bulges),1)
    ...
    def test_stack_found(self, data):
        cg = ftmc.CoarseGrainRNA("test/forgi/threedee/data/stacking/"+data.filename)
        if data.bp1 is None:
            return
        try:
            bulge = self.bulge_from_seqids(cg, data.bp1, data.bp2)
        except (AssertionError, ValueError) as e:
            warnings.warn("Different secondary structure for {}".format(data.filename)+str(e))
            return
